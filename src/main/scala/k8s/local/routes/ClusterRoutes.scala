...
import k8s.local.registry.ClusterMembership
    ...
    Route.seal(
      path("cluster") {
        pathEndOrSingleSlash {
          get {
            val ft : Future[ClusterMembership.MembershipInfo] = clusterMembership.ask(ClusterMembership.GetMembershipInfo)(askTimeout)
